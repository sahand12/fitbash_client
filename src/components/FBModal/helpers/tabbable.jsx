/**
 * if the element can be focused by the user via tabbing
 */
...
function hidesContents(element) {
  ...
  // Otherwise we need to check som styles
...
function tabbable(element) {
  ...
  return (isTabIndexNaN || Number(tabIndex) >= 0) && focusable(element, !isTabIndexNaN);
}

export default function findTabbableDescendants(element) {
  return [].slice.call(element.querySelector('*'), 0).filter(tabbable);
}
